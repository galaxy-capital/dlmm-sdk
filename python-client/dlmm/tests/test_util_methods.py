...
def test_util_methods():
    ...
    user = Keypair.from_bytes([3, 65, 174, 194, 140, 162, 138, 46, 167, 188, 153, 227, 110, 110, 82, 167, 238, 92, 174, 250, 66, 104, 188, 196, 164, 72, 222, 202, 150,
                              52, 38, 249, 205, 59, 43, 173, 101, 40, 208, 183, 241, 9, 237, 75, 52, 240, 165, 65, 91, 247, 233, 207, 170, 155, 162, 181, 215, 135, 103, 2, 132, 32, 196, 16])
    new_balance_position = Keypair.from_bytes([32, 144, 75, 246, 203, 27, 190, 52, 136, 171, 135, 250, 125, 246, 242, 26, 67, 40, 71, 23, 206, 192, 101, 86, 155, 59, 121,
                                              96, 14, 59, 50, 215, 212, 236, 210, 249, 79, 133, 198, 35, 7, 150, 118, 47, 206, 4, 220, 255, 79, 208, 248, 233, 179, 231, 209, 204, 139, 232, 20, 116, 66, 48, 2, 49])
    ...
    bins_between_upper_lower = dlmm.get_bins_between_lower_and_upper_bound(
        0, 2)
    ...
    user_positions = next(filter(lambda x: x.public_key ==
                          new_balance_position.pubkey(), positions.user_positions), None)
    if user_positions:
        claim_lm = dlmm.claim_LM_reward(
            new_balance_position.pubkey(), user_positions)
        assert isinstance(claim_lm, Transaction)

        claim_swap_fee = dlmm.claim_swap_fee(
            new_balance_position.pubkey(), user_positions)
